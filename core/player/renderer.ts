...
export default class Renderer {
    _owner: any = undefined;
    _prepared: any = false;
    _undrawFrame: any = undefined;
    _bitmapCache: any = undefined;
    ...
    prepare () {
      ...
      if (this._owner.videoItem.images === undefined || Object.keys(this._owner.videoItem.images).length == 0) {
        this._bitmapCache = { }
        this._prepared = true
        return void 0
      }
      ...
      if (this._bitmapCache === undefined) {
        ...
        for (let imageKey in this._owner.videoItem.images) {
          let src = this._owner.videoItem.images[ imageKey ]
    ...
    clear () {
      const ctx = (this._owner.drawingCanvas || this._owner.container).getContext('2d')

      const areaFrame = {
        x: 0.0,
        y: 0.0,
        width: (this._owner.drawingCanvas || this._owner.container).width,
        height: (this._owner.drawingCanvas || this._owner.container).height
      }
    ...
    drawFrame (frame: any) {
      if (this._prepared) {
        ...
        this._owner.videoItem.sprites.forEach((sprite: any) => {
          let frameItem = sprite.frames[ this._owner.currentFrame ]
          ...
          if (this._owner.globalTransform) {
            ctx.transform(this._owner.globalTransform.a, this._owner.globalTransform.b, this._owner.globalTransform.c, this._owner.globalTransform.d, this._owner.globalTransform.tx, this._owner.globalTransform.ty)
          }
          ...
          let src = this._owner.dynamicImage[ sprite.imageKey ] || this._bitmapCache[ sprite.imageKey ] || this._owner.videoItem.images[ sprite.imageKey ]
          ...
          if (typeof src === 'string') {
            ...
            if (this._owner.dynamicImageTransform[ sprite.imageKey ] !== undefined) {
              ctx.save()

              const concatTransform = this._owner.dynamicImageTransform[ sprite.imageKey ]
    ...
    resetShapeStyles (ctx: any, obj: any) {
      const styles = obj._styles

      if (styles === undefined) { return void 0 }
    ...
    drawBezierElement (ctx: any, currentPoint: any, method: any, args: any) {
      switch (method) {
        case 'M':
